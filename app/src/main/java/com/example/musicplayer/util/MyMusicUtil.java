...
import com.example.musicplayer.activity.ThemeActivity;
...
public class MyMusicUtil {
    ...
    public static List<MusicInfo> getCurPlayList(Context context) {
        ...
        if (playList != Constant.LIST_LASTPLAY) {
    ...
    //按歌手分组
    public static ArrayList<SingerInfo> groupBySinger(List<MusicInfo> list) {
        ...
        for (MusicInfo musicInfo : list) {
            if (musicMap.containsKey(musicInfo.getSinger())) {
                List<MusicInfo> singerList = musicMap.get(musicInfo.getSinger());
                singerList.add(musicInfo);
            } else {
                ArrayList<MusicInfo> temp = new ArrayList<>();
                temp.add(musicInfo);
    ...
    //按专辑分组
    public static ArrayList<AlbumInfo> groupByAlbum(List<MusicInfo> list) {
        Map<String, List<MusicInfo>> musicMap = new HashMap<>();
        ArrayList<AlbumInfo> albumInfoList = new ArrayList<>();
        for (MusicInfo musicInfo : list) {
            if (musicMap.containsKey(musicInfo.getAlbum())) {
                List<MusicInfo> albumList = musicMap.get(musicInfo.getAlbum());
    ...
    //按文件夹分组
    public static ArrayList<FolderInfo> groupByFolder(List<MusicInfo> list) {
        Map<String, List<MusicInfo>> musicMap = new HashMap<>();
        ArrayList<FolderInfo> folderInfoList = new ArrayList<>();
        for (MusicInfo musicInfo : list) {
            if (musicMap.containsKey(musicInfo.getParentPath())) {
                List<MusicInfo> folderList = musicMap.get(musicInfo.getParentPath());
