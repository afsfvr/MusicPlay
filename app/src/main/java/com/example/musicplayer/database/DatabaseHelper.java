...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final int LAST_TABLE_MAX_COUNT = 100;
    ...
    public static final String LRC_PATH_COLUMN = "lrc_path";
    ...
    //音乐表建表语句
    private static final String createMusicTable = "create table if not exists " + MUSIC_TABLE + "("
            + ID_COLUMN + " integer PRIMARY KEY ,"
            + NAME_COLUMN + " text,"
            + SINGER_COLUMN + " text,"
            + ALBUM_COLUMN + " text,"
            + DURATION_COLUMN + " long,"
            + PATH_COLUMN + " text unique,"
            + PARENT_PATH_COLUMN + " text,"
            + LOVE_COLUMN + " integer,"
            + FIRST_LETTER_COLUMN + " text,"
            + LRC_PATH_COLUMN + " text );";

    //创建播放历史表
    private static final String createLastPlayTable = "create table if not exists " + LAST_PLAY_TABLE + " ("
    ...
    //创建歌单表
    private static final String createPlaylistTable = "create table if not exists " + PLAY_LIST_TABLE + " ("
            + ID_COLUMN + " integer PRIMARY KEY autoincrement,"
            + NAME_COLUMN + " text);";

    //创建歌单歌曲表
    private static final String createListinfoTable = "create table if not exists " + PLAY_LISY_MUSIC_TABLE + " ("
            + ID_COLUMN + " integer,"
